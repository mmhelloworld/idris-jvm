...
createLocalVariable : FC -> String -> NamedCExp
createLocalVariable fc name = NmLocal fc $ UN $ Basic name

getCasesWithLabels : (mapper: a -> Core (String, Int, a)) -> (alts: List1 a) -> Core (List1 (String, Int, a, String))
getCasesWithLabels mapper alts = do
    cases <- traverse mapper (toList alts)
    let Just sortedCases = fromList $ sortBy (comparing second) cases
        | Nothing => asmCrash "Unexpected empty switch cases"
    pure $ addEndLabel sortedCases
  where
    updateEndLabel : (String, Int, a) -> String -> (String, Int, a, String)
    updateEndLabel (labelStart, value, expr) labelEnd = (labelStart, value, expr, labelEnd)

    addEndLabel : List1 (String, Int, a) -> List1 (String, Int, a, String)
    addEndLabel cases =
      let labelStarts = fst <$> cases
      in zipWith updateEndLabel cases (lappend (tail labelStarts) (singleton methodEndLabel))
...
switch : {auto stateRef: Ref AsmState AsmState} -> (defaultLabel: String) -> (labels: List1 String)
               -> (cases: List1 Int) -> Core ()
switch defaultLabel labels cases =
  let min = head cases
      max = last cases
...
storeConditionVariable : {auto stateRef: Ref AsmState AsmState} -> Core NamedCExp
storeConditionVariable = do
  let variableName = conditionValueVariableName ++ show !newDynamicVariableIndex
  conditionVariableIndex <- getVariableIndex variableName
  storeVar IInt IInt conditionVariableIndex
  pure $ createLocalVariable emptyFC variableName

getIfExprCases : List a -> Maybe (List1 a)
getIfExprCases xs@(_ :: []) = Just $ toList1 xs
getIfExprCases xs@(_ :: _ :: []) = Just $ toList1 xs
getIfExprCases xs@(_ :: _ :: _ :: []) = Just $ toList1 xs
getIfExprCases _ = Nothing
...
parameters {auto c : Ref Ctxt Defs} {auto s : Ref Syn SyntaxInfo} {auto stateRef: Ref AsmState AsmState}
  mutual
    ...
    assembleExpr isTailCall returnType (NmConCase fc sc [] def) = do
      ignore $ assembleConstructorSwitchExpr sc
      maybe (defaultValue returnType) (assembleExpr isTailCall returnType) def
    assembleExpr _ returnType (NmConCase fc sc [MkNConAlt name _ _ args expr] Nothing) = do
        idrisObjectVariableIndex <- snd <$> assembleConstructorSwitchExpr sc
    ...
    assembleConstructorSwitchExpr : NamedCExp -> Core (String, Int)
    assembleConstructorSwitchExpr (NmLocal _ loc) =
      let variableName = jvmSimpleName loc
      in pure (variableName, !(getVariableIndex variableName))
    assembleConstructorSwitchExpr sc = do
      let idrisObjectVariableName = constructorSwitchObjectVariableName ++ show !newDynamicVariableIndex
      ...
      pure (idrisObjectVariableName, idrisObjectVariableIndex)
    ...
    ifOperator : Int -> String -> Core ()
    ifOperator 0 = ifne
    ifOperator _ = ificmpne

    ifCondition : NamedCExp -> Int -> String -> Core ()
    ifCondition firstValueExpr secondValue nextLabel = do
      assembleExpr False IInt firstValueExpr
      iconst secondValue
      ifOperator secondValue nextLabel

    assembleIfElseIf : (returnType: InferredType) -> FC -> (ifVariableExpr : NamedCExp)
                     -> (alts: List1 a)
                     -> (caseIntMapper: a -> Core (String, Int, a))
                     -> (caseAssembler: NamedCExp -> String -> Int -> a -> String -> Core ())
                     -> Maybe NamedCExp -> Core ()
    assembleIfElseIf {a} returnType fc ifVariableExpr alts caseIntMapper caseAssembler def = do
      cachedIfVariableExpr <-
        case ifVariableExpr of
          NmLocal _ _ => pure ifVariableExpr
          _ => if shouldStoreVariable alts
                  then do
                    assembleExpr False IInt ifVariableExpr
                    storeConditionVariable
                  else pure ifVariableExpr
      cases <- getCasesWithLabels caseIntMapper alts
      let labels = fst <$> cases
      let exprs = second <$> cases
      traverse_ createLabel (toList labels)
      traverse_ (assembleIfCase cachedIfVariableExpr) (toList cases)
     ...
     where
      assembleIfCase : NamedCExp -> (String, Int, a, String) -> Core ()
      assembleIfCase cachedIfVariableExpr (labelStart, value, expr, labelEnd) =
        caseAssembler cachedIfVariableExpr labelStart value expr labelEnd

    assembleSwitch : (returnType: InferredType) -> FC

                   -- optional as the switch expression is already assembled for constructor switches
                   -- before calling this function. It is passed for constant switches.
                   -> (switchExprAsm: Maybe (Core ()))

                   -> (caseIntMapper: a -> Core (String, Int, a))
                   -> (caseAssembler: String -> a -> String -> Core ())
                   -> List a -> Maybe NamedCExp -> Core ()
    assembleSwitch _ fc _ _ _ [] _ = throw $ GenericMsg fc "Empty cases"
    assembleSwitch {a} returnType fc switchExprAsm caseIntMapper caseAssembler (alt :: rest) def = do
      fromMaybe (pure ()) switchExprAsm
      switchCases <- getCasesWithLabels caseIntMapper (alt ::: rest)
      let labels = fst <$> switchCases
      let exprs = second <$> switchCases
      traverse_ createLabel (toList labels)
      defaultLabel <- createDefaultLabel
      switch defaultLabel labels exprs
      traverse_ assembleCase $ toList switchCases
      maybe (assembleMissingDefault returnType fc defaultLabel) (assembleDefault returnType defaultLabel) def
     where
       assembleCase : (String, Int, a, String) -> Core ()
       assembleCase (labelStart, _, expr, labelEnd) = caseAssembler labelStart expr labelEnd

    assembleExprIfConstAlt : InferredType -> NamedCExp -> String -> Int -> NamedConstAlt -> String -> Core ()
    assembleExprIfConstAlt returnType ifVariableExpr ifExprLabelStart ifValue (MkNConstAlt _ expr) ifBodyLabelEnd = do
      ifCondition ifVariableExpr ifValue ifBodyLabelEnd
      ifBodyLabelStart <- newLabel
      createLabel ifBodyLabelStart
      assembleCaseWithScope ifBodyLabelStart ifBodyLabelEnd (assembleExpr True returnType expr)
      labelStart ifBodyLabelEnd

    assembleExprIfConAlt : InferredType -> Int -> NamedCExp -> String -> Int -> NamedConAlt -> String -> Core ()
    assembleExprIfConAlt returnType idrisObjectVariableIndex ifVariableExpr ifExprLabelStart ifValue
      (MkNConAlt _ _ _ args expr) ifBodyLabelEnd = do
        ifCondition ifVariableExpr ifValue ifBodyLabelEnd
        ifBodyLabelStart <- newLabel
        createLabel ifBodyLabelStart
        assembleCaseWithScope ifBodyLabelStart ifBodyLabelEnd (assembleExpr True returnType expr)
        assembleCaseWithScope ifBodyLabelStart ifBodyLabelEnd
          (assembleConCaseExpr returnType idrisObjectVariableIndex args expr)
        labelStart ifBodyLabelEnd

    assembleExprConAlt : InferredType -> Int -> String -> NamedConAlt -> String -> Core ()
    assembleExprConAlt returnType idrisObjectVariableIndex labelStart (MkNConAlt _ _ _ args expr) labelEnd =
      assembleCaseWithScope labelStart labelEnd (assembleConCaseExpr returnType idrisObjectVariableIndex args expr)

    assembleExprConstAlt : InferredType -> String -> NamedConstAlt -> String -> Core ()
    assembleExprConstAlt returnType labelStart (MkNConstAlt _ expr) labelEnd =
    ...
    assembleConstantSwitch returnType IInt fc sc alts def =
        ...
        in
          case getIfExprCases alts of
            Just ifExprCases => assembleIfElseIf returnType fc sc ifExprCases caseIntMapper
              (assembleExprIfConstAlt returnType) def
            Nothing => do
              let switchExprAsm = Just $ assembleExpr False IInt sc
              let caseIntMapper = constantAltIntExpr fc
              assembleSwitch returnType fc switchExprAsm caseIntMapper (assembleExprConstAlt returnType) alts def
    ...
    assembleConstantSwitch returnType constantType fc sc alts def = do
        ...
        assembleConstantSwitch returnType IInt fc (createLocalVariable fc hashCodePositionVariableName)
            (hashPositionSwitchAlts hashPositionAndAlts) def
      where
        constantAltHashCodeExpr : FC -> (Int, NamedConstAlt) -> Core (Int, Int, NamedConstAlt)
    ...
    assembleConCase returnType fc sc alts def = do
        (idrisObjectVariableName, idrisObjectVariableIndex) <- assembleConstructorSwitchExpr sc
        let hasTypeCase = any isTypeCase alts
        let constructorType = if hasTypeCase then inferredStringType else IInt
        variableTypes <- getVariableTypes
        ...
        let constructorDescriptor = "()" ++ getJvmTypeDescriptor constructorType
        invokeMethod InvokeInterface idrisObjectClass constructorGetter constructorDescriptor True
        if hasTypeCase
            then assembleStringConstructorSwitch returnType fc idrisObjectVariableName alts def
            else assembleConstructorSwitch returnType fc idrisObjectVariableName alts def
    ...
    assembleSwitchExprConAlt : InferredType -> Int -> String -> NamedConAlt -> String -> Core ()
    assembleSwitchExprConAlt returnType idrisObjectVariableIndex labelStart (MkNConAlt _ _ _ args expr) labelEnd =
        assembleCaseWithScope labelStart labelEnd
          (assembleConCaseExpr returnType idrisObjectVariableIndex args expr)

    assembleConstructorSwitch : InferredType -> FC -> String -> List NamedConAlt -> Maybe NamedCExp -> Core ()
    assembleConstructorSwitch _ fc _ [] _ = throw $ GenericMsg fc "Empty cases"
    assembleConstructorSwitch returnType fc idrisObjectVariableName alts def = do
        idrisObjectVariableIndex <- getVariableIndex idrisObjectVariableName
        case getIfExprCases alts of
          Just ifExprCases =>
            let ifExpr = createLocalVariable fc idrisObjectVariableName
            in assembleIfElseIf returnType fc ifExpr ifExprCases conAltIntExpr
                 (assembleExprIfConAlt returnType idrisObjectVariableIndex) def
          Nothing =>
            assembleSwitch returnType fc Nothing conAltIntExpr
              (assembleSwitchExprConAlt returnType idrisObjectVariableIndex) alts def

    assembleStringConstructorSwitch : InferredType -> FC -> String -> List NamedConAlt -> Maybe NamedCExp -> Core ()
    assembleStringConstructorSwitch returnType fc idrisObjectVariableName alts def = do
        constantExprVariableSuffixIndex <- newDynamicVariableIndex
        ...
        labelStart switchEndLabel
        addLineNumber lineNumberStart switchEndLabel
        assembleExpr False IInt (createLocalVariable fc hashCodePositionVariableName)
        assembleConstructorSwitch returnType fc hashCodePositionVariableName
          (hashPositionSwitchAlts hashPositionAndAlts) def
      where
