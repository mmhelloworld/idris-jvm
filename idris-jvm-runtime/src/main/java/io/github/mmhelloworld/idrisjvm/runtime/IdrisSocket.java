...
public final class IdrisSocket implements Closeable {
  ...
  private final int socketFamily;
  private int socketType;
  private UnixDomainSocketAddress unixDomainSocketAddress;

  public IdrisSocket(int socketFamily, int socketType, AbstractSelectableChannel channel) throws IOException {
  ...
  public static int getSocketAddressFamily(Object socketAddressPointer) {
    var socketAddress = (SocketAddress) ((Object[]) socketAddressPointer)[0];
  ...
  public static String getUnixSocketAddress(Object socketAddressPointer) {
    var socketAddress = (UnixDomainSocketAddress) ((Object[]) socketAddressPointer)[0];
  ...
  public int bind(int socketFamily, int newSocketType, String hostName, int port) {
    if (socketFamily == AF_UNIX) {
      return bindUnixSocket(hostName);
    } else {
  ...
  private int bindUnixSocket(String hostName) {
    return withExceptionHandling(() -> {
      this.unixDomainSocketAddress = getUnixDomainSocketAddress(hostName);
      var serverSocketChannel = ServerSocketChannel.open(UNIX);
      channel = serverSocketChannel;
  ...
  private static UnixDomainSocketAddress getUnixDomainSocketAddress(String hostName) {
    var socketPath = Paths.createPath(hostName);
  ...
  public int connect(int socketFamily, int newSocketType, String hostName, int port) {
    return withExceptionHandling(() -> {
      if (socketFamily == AF_UNIX) {
        var socketChannel = SocketChannel.open(UNIX);
  ...
  public int getPort() {
    return switch (socketType) {
      case STREAM_SOCKET_TYPE -> ((ServerSocketChannel) channel).socket().getLocalPort();
      case DATAGRAM_SOCKET_TYPE -> ((DatagramChannel) channel).socket().getLocalPort();
      default -> -1;
    };
  }
  ...
  private void initialize(AbstractSelectableChannel newChannel) throws IOException {
    ...
    ByteBuffer writerBuffer = ByteBuffer.allocate(ONE_KB * SOCKET_BUFFER_CAPACITY_IN_KB);
    writerBuffer.flip(); // position=0, limit=0 — nothing to write until data is placed
    ClientSocketWriterState writerState = new ClientSocketWriterState(writerBuffer, new ResettableCountDownLatch(1));
