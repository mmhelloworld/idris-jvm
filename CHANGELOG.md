...
* Two allocation hot spots in the elaborator no longer build intermediate
  lists. The termination checker's size comparisons (`sizeCompareTyCon`,
  `sizeCompareCon`) walk both application spines in lockstep instead of
  materializing each spine with `getFnArgs` (those spine lists were 97% of
  the checker's cons allocation), and the evaluator's `evalMeta` pushes its
  arguments onto the stack in a single pass instead of `map (EmptyFC,) args
  ++ stk`. Together with the `isSuffixOfBy` change below, total sampled
  allocation while typechecking the compiler's own sources drops from 425
  to 392 GiB and cons-cell allocation falls by 40%.
...
* The JVM backend's higher-order specialisation now reaches the standard
  ...
  - Normalization depends only on pass-stable inputs (the argument's own
    inferred type and the frozen narrowable-class set), never on the
    callee's current slot types, which keep evolving during the
    natural-constructor liveness fixpoint. Consulting them let a call-site
    log flip between the liveness pass and final assembly, so a natural
    function could be judged dead while a caller still emitted a natural
    call to it (`NoSuchMethodError` on `Idris.Desugar.mkPrec` during the
    self-hosted build, and a nondeterministic `BootstrapMethodError`).
  - Plan lookups that were quadratic at compiler scale are precomputed:
    the narrowable-class set is maintained incrementally as constructor
    specs are recorded instead of being recomputed from the whole plan per
    call-site argument (and once per definition per fixpoint iteration
    when a plan was installed), and the type-case check runs on just the
    callee's body at the point a new spec would be created instead of
    walking every definition in the program per site. Whole-program
    codegen of the compiler itself had regressed from ~2 minutes to over
    55; it is back to ~3 minutes.

* Constructor-family interfaces are no longer empty markers: each family
  interface now declares the union of its members' typed accessors, which
  both specialised classes (field reads) and natural constructor classes
  (unboxing bridges over their `Object` fields) implement. Consumers read
  through `invokeinterface` on the family interface, so case binders bound
  from a family-typed discriminant get primitive types regardless of which
  carriers are live. Previously typed reads required knowing the exact
  specialised class and were gated on the natural carrier being dead, so
  any mix of specialised and natural carriers forced generic consumers back
  to boxed `getProperty` reads (allocating per read on specialised cells)
  and kept downstream construction sites from being specialised in turn.
...
* Trampoline frame operations use primitive-typed signatures and a
  ...
  workload from ~2.2x to ~1.8-1.9x (about 1.6x after the allocation
  reductions above).
...
* `Data.List.isSuffixOfBy` (and therefore `isSuffixOf`) no longer reverses
  both lists: it compares lengths and checks `isPrefixOfBy` against the
  dropped tail, allocating nothing. This was the dominant source of
  `reverseOnto` allocation in the compiler via `Namespace.isParentOf`.

### Other changes

* New `profiling/` scripts for measuring pure elaboration (`--typecheck`
  on a pristine copy of the compiler sources) and comparing the JVM
  backend against a Chez reference build of the same commit: interleaved
  timing rounds with load-poisoning detection, JFR CPU and allocation
  reports by phase, method, class and allocation site, and a caller
  attribution tool that skips trampoline/currying plumbing frames. See
  `profiling/README.md` and `docs/generated-code-performance.md`.
